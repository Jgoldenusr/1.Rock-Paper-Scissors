...
function transition(usrPick, usrResult, comPick, compResult) {
  ...
  switch (usrPick) {
    case "piedra":
      userRockButton.classList.add(`${usrResult}`);
      break;
    case "papel":
      userPaperButton.classList.add(`${usrResult}`);
      break;
    case "tijera":
      userScissorsButton.classList.add(`${usrResult}`);
      break;
  }
  switch (comPick) {
    case "piedra":
      computerRockButton.classList.add(`${compResult}`);
      break;
    case "papel":
      computerPaperButton.classList.add(`${compResult}`);
      break;
    case "tijera":
      computerScissorsButton.classList.add(`${compResult}`);
      break;
  }
}
...
function playGame() {
  ...
  if (userScore < 5 && computerScore < 5) {
    ...
    //if computer wins
      ...
      (compOption === "piedra" && userOption === "tijera") ||
    ...
    //if draws
      ...
      round++;
      logArray.push(
        `¡EMPATE en el round ${round}!, ambos usaron ${userOption}, ${userScore}-${computerScore}`
      );
      showLog();
    }
  }
